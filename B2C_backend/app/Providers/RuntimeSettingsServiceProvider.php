...
class RuntimeSettingsServiceProvider extends ServiceProvider
{
    ...
    private function applyStoreSettings(SettingsService $settings): void
    {
        config([
            ...
            'store.shipping.standard_rate' => (float) $settings->get('shipping.standard_rate', $settings->get('shipping.fallback_standard_amount', (float) config('store.shipping.standard_rate', 8))),
            'store.shipping.express_rate' => (float) $settings->get('shipping.express_rate', $settings->get('shipping.fallback_express_amount', (float) config('store.shipping.express_rate', 14))),
            ...
            'store.nzpost.enabled' => $settings->boolean('shipping.nzpost_enabled', $settings->boolean('nzpost.enabled', (bool) config('store.nzpost.enabled', false))),
