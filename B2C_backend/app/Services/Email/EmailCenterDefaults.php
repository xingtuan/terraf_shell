...
class EmailCenterDefaults
{
    ...
    public static function events(): array
    {
        return [
            ...
            ['key' => 'order.created', 'category' => 'Store', 'name' => 'Order created', 'recipient_type' => 'user', 'enabled' => true],
            ['key' => 'order.cancelled', 'category' => 'Store', 'name' => 'Order cancelled', 'recipient_type' => 'user', 'enabled' => false],
            ['key' => 'order.status_changed', 'category' => 'Store', 'name' => 'Order status changed', 'recipient_type' => 'user', 'enabled' => false],
            ['key' => 'order.shipped', 'category' => 'Store', 'name' => 'Order shipped', 'recipient_type' => 'user', 'enabled' => false],
            ['key' => 'order.admin_new_order', 'category' => 'Store', 'name' => 'Admin new order alert', 'recipient_type' => 'admin', 'enabled' => true],
    ...
    public static function subject(string $key, string $locale = 'en'): string
    {
        $subjects = [
            ...
            'order.created' => 'Order {{ order.order_number }} received',
            'order.cancelled' => 'Order {{ order.order_number }} was cancelled',
            'order.status_changed' => 'Order {{ order.order_number }} status update',
            'order.shipped' => 'Order {{ order.order_number }} has shipped',
            'order.admin_new_order' => 'New order {{ order.order_number }}',
    ...
    public static function html(string $key): string
    {
        return match (true) {
            ...
            str_contains($key, 'admin_new_order') => '<p>A new order was placed.</p><p>Order: {{ order.order_number }}</p><p>Customer: {{ user.name }} {{ user.email }}</p><p>Total: {{ order.total }} {{ order.currency }}</p>',
            str_starts_with($key, 'order.') => '<p>Hello {{ user.name }},</p><p>Order {{ order.order_number }} is currently {{ order.status }}.</p><p>Total: {{ order.total }} {{ order.currency }}</p><p>{{ order.items }}</p><p><a href="{{ order_url }}">View order</a></p><p>Shipping: {{ shipping.address }}</p>',
