...
class EmailCenterDefaults
{
    ...
    public static function html(string $key): string
    {
        return match (true) {
            ...
            str_contains($key, 'admin_new_order') => '<p>A new order request was submitted.</p><p>Order request: {{ order.order_number }}</p><p>Customer: {{ user.name }} {{ user.email }}</p><p>Customer type: {{ order.customer_type }}</p><p>Shipping method: {{ order.shipping_method }}</p><p>Total estimate: {{ order.total }} {{ order.currency }}</p><p>Payment will be handled manually.</p>',
    ...
    public static function variables(string $key): array
    {
        ...
        return array_values(array_unique(array_merge($common, match (true) {
            ...
            str_contains($key, 'inquiry') || str_contains($key, 'lead') || str_contains($key, 'sample_request') => ['inquiry.name', 'inquiry.email', 'inquiry.company', 'inquiry.message', 'inquiry.type', 'inquiry.interest_type', 'inquiry.application', 'inquiry.estimated_quantity', 'inquiry.timeline', 'inquiry.custom_fields', 'inquiry.custom_fields_text', 'lead.reference', 'lead.status', 'lead.interest_type', 'lead.application_type', 'lead.expected_use_case', 'lead.estimated_quantity', 'lead.timeline', 'lead.custom_fields', 'lead.custom_fields_text', 'assignee.name'],
