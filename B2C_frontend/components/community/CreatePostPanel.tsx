...
import { createPost, listCategories, updatePost } from "@/lib/api/posts"
...
type FieldErrors = Partial<
  Record<
    "title" | "category_id" | "tags" | "content" | "funding_url" | "images",
    string
  >
>
...
export function CreatePostPanel({
  ...
  useEffect(() => {
    ...
    setFundingUrl(initialData?.funding_url ?? "")
    setImages([])
    setErrors({})
    setFormError(null)
  }, [initialData, open])
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="right" className="w-full overflow-y-auto sm:max-w-2xl">
                ...
                .catch((error) => {
                  if (error instanceof ApiError) {
                    setErrors({
                      ...
                      tags: getApiFieldError(error, ["tags"]),
                      content: getApiFieldError(error, ["content"]),
        ...
        >
          ...
          {isEditing && initialData?.images.length ? (
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                {initialData.images.map((image) => (
                  ...
                  >
                    <img
                      src={image.url || getCommunityPostCoverImage(initialData)}
                      alt={image.alt_text ?? initialData.title}
